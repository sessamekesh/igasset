...
DracoDecoder::get_pos_norm_data() const {
  ...
  for (draco::PointIndex vert_idx(0); vert_idx < mesh_->num_points();
       vert_idx++) {
    PositionNormalVertexData3D vert{};
...
DracoDecoder::get_texcoord_data() const {
  if (texcoord_attrib_ < 0) {
    return DracoDecoderError{DracoDecoderErrorType::MeshDataMissing,
                             "Invalid texcoord_attrib index value"};
  }

  auto texcoord_attrib = mesh_->GetNamedAttributeByUniqueId(
      draco::GeometryAttribute::TEX_COORD, texcoord_attrib_);

  if (!texcoord_attrib) {
    return DracoDecoderError{DracoDecoderErrorType::MeshDataMissing,
                             "Attribute at texcoord is missing"};
  }

  if (texcoord_attrib->num_components() != 2 ||
      texcoord_attrib->data_type() != draco::DataType::DT_FLOAT32) {
    return DracoDecoderError{DracoDecoderErrorType::MeshDataInvalid,
                             "Invalid texcoord buffer format"};
  }
...
std::variant<IndexBuffer, DracoDecoderError> DracoDecoder::get_index_buffer()
    const {
  int num_faces = mesh_->num_faces();
  IndexBuffer::Builder builder(index_buffer_type_, num_faces * 3u);
  for (draco::FaceIndex idx(0); idx < num_faces; idx++) {
    ...
    for (const auto& face_idx : face) {
      uint32_t index_value = face_idx.value();
